...
export class PlyViewer2Component implements OnInit, OnDestroy {
  ...
  private onMouseDown = (event: MouseEvent) => {
    ...
    if (ringIntersects.length > 0) {
      ...
      if (this.boundingBoxEditData[0]) {
        this.rotationCenter = new THREE.Vector3(
          this.boundingBoxEditData[0].centerX,
          this.boundingBoxEditData[0].centerY,
          this.boundingBoxEditData[0].centerZ
        );
      } else {
      ...
      this.initialRotation = {
        x: this.boundingBoxEditData[0].rotationX,
        y: this.boundingBoxEditData[0].rotationY,
        z: this.boundingBoxEditData[0].rotationZ
      };
    ...
    if (cornerIntersects.length > 0) {
      ...
      if (this.boundingBoxEditData[0]) {
        this.boundingBoxEditData[0].originalCenter = new THREE.Vector3(
          this.boundingBoxEditData[0].centerX,
          this.boundingBoxEditData[0].centerY,
          this.boundingBoxEditData[0].centerZ
        );
        this.boundingBoxEditData[0].originalDimensions = new THREE.Vector3(
          this.boundingBoxEditData[0].dimensionX,
          this.boundingBoxEditData[0].dimensionY,
          this.boundingBoxEditData[0].dimensionZ
        );
      }
    }
  }
  ...
  private onMouseMove = (event: MouseEvent) => {
    if (!this.isEditMode || (!this.selectedCorner && !this.selectedRing) || !this.boundingBoxMesh || !this.boundingBoxEditData[0]) return;
  ...
  private handleRingRotation(raycaster: THREE.Raycaster) {
    if (!this.boundingBoxEditData[0] || !this.selectedRing || !this.rotationCenter) return;
    ...
    if (raycaster.ray.intersectPlane(plane, intersectionPoint)) {
      ...
      switch (this.cornerEditMode) {
        case CornerEditMode.RotationX:
          this.boundingBoxEditData[0].rotationX = this.initialRotation.x + angleDegreesDifference;
          break;
        case CornerEditMode.RotationY:
          this.boundingBoxEditData[0].rotationY = this.initialRotation.y + angleDegreesDifference;
          break;
        case CornerEditMode.RotationZ:
          this.boundingBoxEditData[0].rotationZ = this.initialRotation.z + angleDegreesDifference;
          break;
      }
  ...
  private updateBoundingBoxMesh() {
    if (!this.boundingBoxEditData[0] || !this.boundingBoxMesh) return;
    
    // Update the bounding box mesh rotation
    this.boundingBoxMesh.rotation.set(
      this.boundingBoxEditData[0].rotationX * (Math.PI / 180),
      this.boundingBoxEditData[0].rotationY * (Math.PI / 180),
      this.boundingBoxEditData[0].rotationZ * (Math.PI / 180)
    );
  ...
  private updateRotationRings() {
    if (!this.boundingBoxEditData[0] || this.rotationRings.length !== 3) return;
  ...
  private handleCornerMove(raycaster: THREE.Raycaster) {
    if (!this.boundingBoxEditData[0] || !this.selectedCorner) return;
  ...
  private updateDimensionMaintainingNormalizedScale(newPosition: THREE.Vector3, axis: 'x' | 'y' | 'z') {
    if (!this.boundingBoxEditData[0] || !this.selectedCorner || !this.boundingBoxEditData[0].originalDimensions) return;
    ...
    switch (axis) {
      case 'x':
        newDimension = Math.max(0.1, this.boundingBoxEditData[0].originalDimensions.x + delta * 2);
        this.boundingBoxEditData[0].dimensionX = newDimension;
        
        // Update Y and Z dimensions to maintain normalized scale ratios
        if (this.boundingBoxEditData[0].normalizedRatioY) {
          this.boundingBoxEditData[0].dimensionY = newDimension * this.boundingBoxEditData[0].normalizedRatioY;
        }
        if (this.boundingBoxEditData[0].normalizedRatioZ) {
          this.boundingBoxEditData[0].dimensionZ = newDimension * this.boundingBoxEditData[0].normalizedRatioZ;
        }
        break;
        
      case 'y':
        newDimension = Math.max(0.1, this.boundingBoxEditData[0].originalDimensions.y + delta * 2);
        this.boundingBoxEditData[0].dimensionY = newDimension;
        
        // Update X and Z to maintain normalized scale ratios
        if (this.boundingBoxEditData[0].normalizedRatioY) {
          const scaleFactor = newDimension / this.boundingBoxEditData[0].normalizedRatioY;
          this.boundingBoxEditData[0].dimensionX = scaleFactor;
          if (this.boundingBoxEditData[0].normalizedRatioZ) {
            this.boundingBoxEditData[0].dimensionZ = scaleFactor * this.boundingBoxEditData[0].normalizedRatioZ;
          }
        }
        break;
        
      case 'z':
        newDimension = Math.max(0.1, this.boundingBoxEditData[0].originalDimensions.z + delta * 2);
        this.boundingBoxEditData[0].dimensionZ = newDimension;
        
        // Update X and Y to maintain normalized scale ratios
        if (this.boundingBoxEditData[0].normalizedRatioZ) {
          const scaleFactor = newDimension / this.boundingBoxEditData[0].normalizedRatioZ;
          this.boundingBoxEditData[0].dimensionX = scaleFactor;
          if (this.boundingBoxEditData[0].normalizedRatioY) {
            this.boundingBoxEditData[0].dimensionY = scaleFactor * this.boundingBoxEditData[0].normalizedRatioY;
          }
        }
        break;
    }
  ...
  private updateCenter(newPosition: THREE.Vector3) {
    if (!this.boundingBoxEditData[0] || !this.boundingBoxEditData[0].originalCenter || !this.selectedCorner) return;
    ...
    this.boundingBoxEditData[0].centerX = this.boundingBoxEditData[0].originalCenter.x + deltaX;
    this.boundingBoxEditData[0].centerY = this.boundingBoxEditData[0].originalCenter.y + deltaY;
    this.boundingBoxEditData[0].centerZ = this.boundingBoxEditData[0].originalCenter.z + deltaZ;
  }
  ...
  private createRotationRings() {
    if (!this.boundingBoxEditData[0]) return;
  ...
  private updateRotationRingsTransform() {
    ...
    const center = new THREE.Vector3( this.boundingBoxEditData[0].centerX, this.boundingBoxEditData[0].centerY, this.boundingBoxEditData[0].centerZ);
    ...
    const boxRotation = new THREE.Euler(
      this.boundingBoxEditData[0].rotationX,
      this.boundingBoxEditData[0].rotationY,
      this.boundingBoxEditData[0].rotationZ,
      'XYZ'
    );
  ...
  private createBoundingBoxCorners() {
    ...
    // Store normalized dimension ratios for proportional scaling
    if (!this.boundingBoxEditData[0].normalizedRatioY) {
      this.boundingBoxEditData[0].normalizedRatioY = dimensions.y / dimensions.x;
    }
    
    if (!this.boundingBoxEditData[0].normalizedRatioZ) {
      this.boundingBoxEditData[0].normalizedRatioZ = dimensions.z / dimensions.x;
    }
    ...
    try {
      ...
      for(let i = 0; i < data.length; i++){
        ...
        this.boundingBoxEditData.push({
          ...
          originalCenter: new THREE.Vector3(bboxData.center_cam[0], bboxData.center_cam[1], bboxData.center_cam[2]),
          originalDimensions: new THREE.Vector3(
            bboxData.dimensions[0],
            bboxData.dimensions[1],
            bboxData.dimensions[2]
          ),
